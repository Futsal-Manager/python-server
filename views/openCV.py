...
# For Highlight
BALL_SPEED_MINIMUM = 15
BALL_SPEED_MAXIMUM = 30
...
# 공 (주황)
orangeBallLower = (0, 50, 70)
orangeBallUpper = (25, 80, 120)
...
def findGoalPostByColorAndDirection(yellowColor, orangeColor):
    ...
    if (findSpotResult != None):
        (
            (mask, cnts, c1, ((x1, y1), radius1), center1),
            (mask, cnts, c2, ((x2, y2), radius2), center2)) = findSpotResult
        # 노란색 마커에 초록색 동그라미
    ...
    if (findSpotResult != None):
        (
            ...
            (mask, cnts, c2, ((x2, y2), radius2), center2)) = findSpotResult
        # 주황색 마커에 초록색 동그라미
        cv2.circle(frame, center1, 20, (0, 255, 0), 10)
        cv2.circle(frame, center2, 20, (0, 255, 0), 10)
        # cv2.putText(frame, 'Yellow Center 1: ' +str(center1), center1, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 2)
        # cv2.putText(frame, 'Yellow Center 2: ' + str(center2), center2, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        # cv2.imshow('mask', mask) # Just Yellow
    ...
    if len(yellowCenterArr) >= 2 and len(orangeCenterArr) >= 2:  # 만약 2개의 점을 모두 검출하면 정렬,
        return _makeCenterBeetweenColor(yellowCenterArr, orangeCenterArr)  # 골대 외곽선 그리기
    else:
        return None
...
def findBallColor(lower, upper):
    ...
    if findSpotResult is not None:
        ...
        if BALL_MIN_RADIUS < radius and radius < BALL_MAX_RADIUS:
            ...
            return _drawBallTrackLine(center)  # 볼의 선을 그려서 경로 알아봄, 속력 리턴
        else:
            return None
    else:
        return None
...
def _findSpot(lower, upper):
    ...
    hsvCopy = hsvCopy[0: Y_DIMENSION / 3, 0: X_DIMENSION]
    mask = cv2.inRange(hsvCopy, hsvConverter(lower), hsvConverter(upper))
    # cv2.imshow('Original ball post marker ', mask)
    # 컨투어 찾아내기
    ...
    if cnts:
        ...
        # Todo: Critical Part
        # while radius1 > 10 or radius2 > 10 and idx + 1 < len(sortedCnts) - 1 and len(sortedCnts) > 2:
...
def _findBallColor(lower, upper):
    hsvCopy = np.array(hsv)  # copy
    hsvCopy = hsvCopy[Y_DIMENSION / 3: Y_DIMENSION, 0: X_DIMENSION]
    ...
    if cnts:
        ...
        if c is not None:
            ...
            if M["m00"] != 0 and M["m00"] != 0:
                center = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]) + Y_DIMENSION / 3)
...
def _makeCenterBeetweenColor(_yellowCenterArr, _orangeCenterArr):  # arr의 length는 항상 4
    ...
    # Todo: 골대의 중심좌표를 반환
    avgXpos = (leftTopCenter[0] + rightTopCenter[0] + rightBottomCenter[0] + leftBottomCenter[0]) / 4
    avgYpos = (leftTopCenter[1] + rightTopCenter[1] + rightBottomCenter[1] + leftBottomCenter[1]) / 4
    return (avgXpos, avgYpos)
...
def _drawBallTrackLine(center):
    ...
    if center is not None and prevCenter is not None:
        if prevCenter[0] - center[0] > 0:
            ...
            retSpeed = prevCenter[0] - center[0]
        elif center is not None:
            print '오른쪽 속력: ', center[0] - prevCenter[0]
            retSpeed = center[0] - prevCenter[0]
    ...
    return (retSpeed, center)


# './sample4.mp4'
def processing(videoPath):  # videoPath
    ##################################################
    ############### Main Function ####################
    ##################################################
    cap = cv2.VideoCapture(videoPath)  # samplevideo.mp4
    fps = cap.get(cv2.CAP_PROP_FPS)
    frameTotal = cap.get(cv2.CAP_PROP_FRAME_COUNT)
    print "This video fps is:", fps
    print "frameTotal", frameTotal

    # Note: milisecond로 프레임을 넘김
    ...
    while True:
        ...
        # Start time
        start = time.time()
        global grabbed, hsv, yellowCenterArr, orangeCenterArr
        ...
        if MODE == WEBCAM_MODE:
            ...
            ret, frame = cap.read()
            mesc = cap.get(cv2.CAP_PROP_POS_MSEC)  # 영상의 milisecond
            sec = mesc * 0.001
            print "timestamp(sec): " + "now: " + str(sec) + " / Total: " + str(frameTotal / fps)
            print "Percent", (float(sec) / float(frameTotal / fps) * 100), "%"
        ...
        # 주황(공) 위한 mask
        ballRes = findBallColor(orangeBallLower, orangeBallUpper)
        # 4. 마커 Like한 아이들 찾기
        markerAvgCenterPos = findGoalPostByColorAndDirection((yellowLower, yellowUpper),
                                                             (orangePostLower, orangePostUpper))

        if (ballRes):
            (ballSpeed, ballCenterPos) = ballRes
            if ballSpeed and ballCenterPos and markerAvgCenterPos:
                print 'ballCenterPos is', ballCenterPos
                print 'markerAvgCenterPos is', markerAvgCenterPos
                # X좌표: markerAvgCenterPos[0], ballCenterPos[0]
                # Y좌표: markerAvgCenterPos[1], ballCenterPos[1]
                # 왼쪽 위 상단 (0,0)

                # 공속도가 일정 범위 이내
                if BALL_SPEED_MINIMUM < ballSpeed and ballSpeed < BALL_SPEED_MAXIMUM:
                    # 공이 오른쪽 또는 왼쪽에 있을때 일정 높이 이내
                    if (markerAvgCenterPos[0] - ballCenterPos[0]) > 0 and (
                                ballCenterPos[1] - markerAvgCenterPos[1]) > 0 or (
                                ballCenterPos[0] - markerAvgCenterPos[0]) > 0 and (
                        ballCenterPos[1] - markerAvgCenterPos[1]) > 0:
                        mesc = cap.get(cv2.CAP_PROP_POS_MSEC)  # 영상의 milisecond
                        sec = int(mesc * 0.001)
                        print "시간 기록" + str(sec)
                        setTime.add(sec)
    ...
    # 하이라이트 시간대 출력
    sortedSet = sorted(setTime)
    print "하이라이트 시간대" + str(sortedSet)
